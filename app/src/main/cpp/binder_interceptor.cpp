...
protected:
    status_t onTransact(uint32_t code, const Parcel &data, Parcel *reply, uint32_t flags) override {
        ...
        if (!interceptorManagedFlow) {
            LOGD("[TX_ID: %" PRIu64 "] Forwarding to original implementation.", info.transaction_id);
...
void inspectAndRewriteTransaction(binder_transaction_data *txn_data) {
    ...
    if (hijack) {
        ...
        LOGD("[Hook] Hijacking Transaction %" PRIu64 " (Code: %u)", tx_id, txn_data->code);
...
void processBinderReadBuffer(const binder_write_read &bwr) {
    ...
    while (ptr < end) {
        ...
        LOGD("[Driver -> User] Command: %s (0x%x), DataSize: %zu", getBinderReturnCommandName(cmd), cmd, cmd_size);
